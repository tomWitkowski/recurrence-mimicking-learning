...
class Decoder(tf.keras.Model):
    ...
    def __init__(self, layer_shape, activation=tf.keras.layers.LeakyReLU(0.1), K: int = 3):
        super().__init__()
        z_in = z_out_layer = tf.keras.Input(shape=(layer_shape))

        prev_action_in = tf.keras.Input((K))
        merged = tf.concat([z_out_layer, prev_action_in], 1)
...
class DummyUtilityFunction(tf.keras.Model):
    """
    Dummy utility function for inputs of any K returning just average of all inputs
    """
...
class Agent:
    ...
    K: number of possible actions (3: short, neutral, long).
    """
    ...
    def phi_processing(self, stacked_preds, initial_action=1):
        lines = tf.concat(tf.split(tf.round(stacked_preds)+1, self.K), 1)
    ...
    def recurrence_mimicking_forward_pass(self, XV):
        ...
        print(stacked_z.shape, stacked_a.shape)
    ...
    def train_iteration(self, XV, BA, offline=False, online_learning=False, count_time=False, forward_only=False, with_gradients = None):
        if with_gradients is None:
            with_gradients = not forward_only
        self.time_counter = []
        if online_learning:
            dec_seq = tf.constant([1.])
            A, B, eta = 0., 0.01, 0.05
            for i, xrow in enumerate(XV[:-1]):
                tape_object = nullcontext() if forward_only else tf.GradientTape()
                with tape_object as tape:
                    prev_a = tf.one_hot(tf.cast(tf.round(dec_seq[-1:,None]), tf_int),self.K)[0,...]
            ...
            tape_object = tf.GradientTape() if with_gradients else nullcontext()
            with tape_object as tape:
                dec_seq = tf.constant([0.], dtype=tf_float)
                start = time.time()
                for i,xrow in enumerate(XV):
                    prev_a = tf.one_hot(tf.cast(tf.round(dec_seq[-1:,None])+1.0, tf_int),self.K)[0,...]
