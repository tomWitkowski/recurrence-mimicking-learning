...
#         all_dec = tf.concat( [ tf.concat([ones, zeros, zeros],1), tf.concat([zeros, ones, zeros],1), tf.concat([zeros, zeros, ones],1) ],0)
        ...
        if f'DF_{CHANGE}_{LENGTH}.csv' not in os.listdir():    
            print('Calculate DF')
            if False:
                data = pd.concat([
            ...
            DF.to_csv(f'DF_{CHANGE}_{LENGTH}.csv', index=False)
    ...
    #         DF = pd.concat([DF, concated_other['y']],1).dropna()
            ...
            DF = pd.read_csv(f'DF_{CHANGE}_{LENGTH}.csv')
        ...
        for cat in ['train_data','test_data']:
        ...
        DF = pd.read_csv('input_table_trans_data_aud_jpy.csv', index_col=0)
...
def get_done_data(cat: str = 'train_data'):
